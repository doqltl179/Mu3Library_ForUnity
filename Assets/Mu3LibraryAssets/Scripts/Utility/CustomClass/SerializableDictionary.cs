...
namespace Mu3Library.Utility.CustomClass {
    [Serializable]
    public class SerializableDictionary<T> : Dictionary<string, T>, ISerializationCallbackReceiver where T : class {
        [Serializable]
        ...
        private struct NonAssetSaveItem {
            ...
            public NonAssetSaveItem(string key, T obj, int index) {
                this.key = key;
                this.obj = obj;
                this.index = index;
            }
                ...
                switch(value) {
                    ...
                    case UnityEngine.Object obj:
                        ...
                        if(!string.IsNullOrEmpty(assetPath)) {
                            ...
                            sceneObjectSaveItems.Add(new NonAssetSaveItem(keys[i], obj as T, i));
                        }
                        break;
        ...
        private void DeserializeItem(string contentType, string key, string encodedValue) {
            switch(contentType) {
                ...
                case "int": Add(key, (int)int.Parse(encodedValue) as T); return;
                case "float": Add(key, (float)float.Parse(encodedValue) as T); return;
                case "double": Add(key, (double)double.Parse(encodedValue) as T); return;
                case "long": Add(key, (long)long.Parse(encodedValue) as T); return;
                case "string": Add(key, (string)encodedValue as T); return;
                case "bool": Add(key, (bool)(encodedValue == "true" ? true : false) as T); return;
                case "Vector2": Add(key, Serializer.ParseVector2(encodedValue) as T); return;
                case "Vector3": Add(key, Serializer.ParseVector3(encodedValue) as T); return;
                case "Vector2Int": Add(key, Serializer.ParseVector2Int(encodedValue) as T); return;
                case "Vector3Int": Add(key, Serializer.ParseVector3Int(encodedValue) as T); return;
                case "Vector4": Add(key, Serializer.ParseVector4(encodedValue) as T); return;
                case "Bounds": Add(key, Serializer.ParseBounds(encodedValue) as T); return;
                case "Rect": Add(key, Serializer.ParseRect(encodedValue) as T); return;
                case "Color": Add(key, JsonUtility.FromJson<Color>(encodedValue) as T); return;
                case "AnimationCurve": Add(key, Serializer.DeserializeAnimationCurve(encodedValue) as T); return;
                case "Gradient": Add(key, Serializer.DeserializeGradient(encodedValue) as T); return;
                case "UnityEngine.Object":
                    if(Application.isEditor) {
                        EditorApplication.delayCall += () => Add(key, AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(encodedValue) as T);
                    }
                    else {
                        Add(key, Resources.Load(encodedValue) as T);
                    }
                    return;
                default: break;
            }
            ...
            if(Serializer.EnumDeserialize(contentType, encodedValue, out T enumValue)) {
                Add(key, enumValue);
            }

            //Tries to de-serialize a struct or class using JsonUtility.FromJson
            if(Serializer.TryDeserializeJSON(contentType, encodedValue, out T result)) {
                Add(key, result);
            }
        }

        public void SetOrAdd(string key, T ob) {
        ...
        private static class Serializer {
            ...
            /// </summary>
            public static bool EnumDeserialize(string contentType, string encodedValue, out T enumValue) {
                Type type = Type.GetType(contentType);
                if(type != null && type.IsEnum) {
                    if(Enum.TryParse(type, encodedValue, out object enumIntermediateValue)) {
                        enumValue = Convert.ChangeType(enumIntermediateValue, type) as T;
                        return true;
                    }
                }
                enumValue = null;
            ...
            /// </summary>
            public static bool TryDeserializeJSON(string contentType, string json, out T result) {
                Type type = Type.GetType(contentType);
                if(type != null) {
                    result = JsonUtility.FromJson(json, type) as T;
                    if(result != null) {
                        return true;
                    }
                }
    ...
    [Serializable]
    public class SerializableDictionary : SerializableDictionary<object> { }
}
