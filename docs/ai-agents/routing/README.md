...
|---|---|---|---|
| `orchestrator` | Governance | Credit-aware node admission, dependency-graph decomposition, ownership, readiness, dispatch waves, gates | [spec](../../../.github/agents/orchestrator.agent.md) |
...
| `task-planner` | Execution | Current-unit plans plus orchestrator-approved graph records and progress state | [spec](../../../.github/agents/task-planner.agent.md) |
...
| `release-manager` | Execution | Plan-declared graph Git lifecycle/fan-in, versioning, tags, branch sync, and GitHub Releases | [spec](../../../.github/agents/release-manager.agent.md) |
| `cli-platform` | Execution | Repository-local Python and CLI tooling | [spec](../../../.github/agents/cli-platform.agent.md) |
| `reviewer` | Quality | Node/fan-in evidence, regression, compatibility, docs, release, and verification review | [spec](../../../.github/agents/reviewer.agent.md) |
...
| Complex request has two or more independent writable units | `orchestrator` | the graph is closed/cancelled or collapses to serial before worktree creation; retain control while any graph node/resource remains active |
| Current unit needs a step plan, progress tracking, or an approved graph update recorded | `task-planner` | topology, dependency, scope, readiness, allocation, cross-agent routing, structural fit, or quality decisions are needed |
