...
### 追加
- Base パッケージに新しいオプションアセンブリ `Mu3Library.Notifications` が加わりました。`INotificationManager`/`NotificationManager` が Mobile Notifications パッケージの統合 `NotificationCenter` を包みます — Android チャンネルを含む初期化、権限リクエスト、時刻・遅延でのスケジュール、キャンセル、バッジのクリア、システム設定を開く操作、アプリを開いた通知の取得まで。`INotificationManagerEventBus.OnNotificationReceived` はアプリ実行中に届いた通知を知らせます。このアセンブリは Android・iOS・エディターのみ、かつ `com.unity.mobile.notifications` がインストールされている間だけ(`MU3LIBRARY_NOTIFICATIONS_SUPPORT`)コンパイルされるため、パッケージやプラットフォームのないプロジェクトは何のコストも払いません。パッケージは yield 方式の権限・照会オペレーションに対してイベントを push しないため、マネージャーは各オペレーションを、そのオペレーションが生きている間だけ存在する短い async ループで見守ります — 待機中のものがなければ何もポーリングせず、フレーム単位のドライバーなしに、どのようにホストされても同じように動作します。UniTask がインストールされていれば `RequestPermissionAsync` と `GetLastRespondedNotificationAsync` で同じ答えを await できます。Template サンプルに `NotificationCore` が追加されました。
...
### 修正
- `SafeRect`: edit mode で rect が毎フレーム新しい大きさになる問題を修正しました。`ScreenChangeNotifier` がエディタループから画面を読んでおり、そこでの `Screen` はエディタが描いている view を返します。あるティックでは game view、次のティックでは scene view や inspector でした。そのためすべてのティックが「たった今変わった画面」に見え、画面では何も変わっていないのに毎回異なる 2 つの値から safe area が適用し直されていました。
...
### 変更
- `ILocalizationManager`: `AddLocaleChangedEvent(Action<Locale>)` と `RemoveLocaleChangedEvent(Action<Locale>)` を削除しました。ロケール変更の購読はイベントバスの担当なので、`ILocalizationManagerEventBus.OnLocaleChanged` が両方を置き換えます。マネージャーを `ILocalizationManagerEventBus` として解決し、`+=` と `-=` を使ってください。
- `ILocalizationManager`: `GetAllKeys` は `IReadOnlyList<string>` を、`GetAvailableLocales` は `IReadOnlyList<Locale>` を返します。これまでの `GetAvailableLocales` は Localization パッケージが保持する `List<Locale>` そのものを渡しており、呼び出し側が並べ替えたり空にしたりできました。
- `Mu3Library.Localization.Data`: `EntryData`、`LocaleData`、`TableData` を `Mu3Library.Addressable.Data` と同じく `MU3LIBRARY_LOCALIZATION_SUPPORT` で囲みました。エクスポーターが生成するスクリプトにはこのガードがないため、define を切ったまま生成済みの Localization スクリプトを残しているプロジェクトは、そのスクリプトも併せて削除する必要があります。
- `LocalizationManager.InitializeAsync()`: 初期化の失敗が await の外へ再スローされなくなりました。コールバック API と同じく `IsInitialized`、`InitializeError`、`OnInitializeResult` を通じて報告され、ログもこれまでどおり残ります。
- `LocalizationManager`: `LocalizationSettings` アセットを持たないプロジェクトには、使い捨ての既定設定を渡す代わりに空文字列、空のリスト、またはフォールバックロケールを返します。`LocalizationSettings` のメンバーを読むとアクセス時に既定の設定オブジェクトが作られ、その警告も出るため、各入口でまず `LocalizationSettings.HasSettings` を確認します。
- `AddressablesManager.InitializeAsync()`: 初期化の失敗が await の外へ再スローされなくなりました。コールバック API と同じく `IsInitialized`、`InitializeError`、`OnInitializeResult` を通じて報告され、ログもこれまでどおり残るため、3 つの入口が 1 つの失敗を同じ形で伝えます。また、ハンドルの取得と完了処理の手順を書き直す代わりに `BeginInitialize()` を通すので、コールバック経路と async 経路が食い違う余地がなくなります。

### 修正
- `LocalizationManager`: カスタムロケールコードで既定ロケールを解決するときや英語名でロケールを探すときに例外が出なくなりました。`LocaleIdentifier.CultureInfo` は culture 表にないコードでは null のままで、Localization パッケージはこれを正常なケースとして文書化していますが、どちらの検索もこの値をそのまま参照していました。
- `LocalizationManager`: `GetSelectedLocale(Action<Locale>)` が、自分で確認したハンドルをそのまま待ちます。完了ハンドラーを登録する際に `LocalizationSettings.SelectedLocaleAsync` を読み直していましたが、その間に `SelectedLocale` を代入すると既存のオペレーションが解放されて新しいものが作られるため、完了を確認したハンドルとコールバックを載せたハンドルが別物になり得ました。今はプロパティを一度だけ読みます。
- `LocalizationManager`: キャンセルされた `ChangeLocaleAsync(Locale)` が、自分を置き換えた変更の完了を報告しなくなりました。追い越された呼び出しが自身の `finally` で `IsLocaleChanging` を下ろしていましたが、その時点では新しい呼び出しがすでに上げた後でした。今はキャンセルソースを保持している呼び出しだけがこの状態を片付けます。
- `LocalizationManager`: `CurrentLocale` がマネージャーの外で起きたロケール変更に追従します。この値を書いていたのはマネージャー自身の変更メソッドだけだったため、`LocalizationSettings.SelectedLocale` を直接設定した場合や起動時のセレクターが選んだ場合には、以前のロケールを報告し続けていました。
- `LocalizationManager`: `ChangeLocaleToNative()` が、enum 名と culture の英語名が一致しないシステム言語も解決します。`SystemLanguage.ToString()` と `CultureInfo.EnglishName` の比較では `ChineseSimplified` が `Chinese (Simplified)` に一致することはありませんでした。今は Localization パッケージが `SystemLanguage` から作る識別子を最初に試し、次に地域を問わない同一言語、最後に英語名を試します。
- `LocalizationManager`: `GetStringAsync(string, string)` がテーブル読み込み失敗時に意図どおり空文字列を返します。await の後ろにあった状態チェックは、失敗したオペレーションを await するとその前に例外が投げられるため到達できませんでした。
- `AddressablesManager`: `InitializeAsync()` が、例外の伝播中に `finally` の中で await しなくなりました。初期化がすでに動いているときに入ってきた呼び出し側はそこで `UniTask.WaitUntil` を待っていましたが、これにはタイムアウトもキャンセルトークンもないため、例外が表に出ないまま待機に縛られていました。
- `AddressablesManager`: `Dispose()` が初期化ハンドルを解放する前に完了ハンドラーを外します。片付けの済んだマネージャーに、遅れて届いた完了が `_isInitialized` を書き戻すことができなくなります。
...
### 修正
- `SafeRect`: anchor が毎フレーム 2 つの値の間で入れ替わり、コンポーネントが所有する rect の大きさが変わり続ける問題を修正しました。従来は `OnRectTransformDimensionsChange` の中で `Screen` を自分で読んでいましたが、このメッセージは canvas の再構築中に送られ、`Screen` は問い合わせたパスのレンダーターゲットを返します。つまり scene view の再描画では scene view の大きさを、game view の再描画では game view の大きさを報告し、どちらの答えも「画面がもう一方に変わった」ように見えていました。現在このコンポーネントが読む画面は `ScreenChangeNotifier` だけなので、1 フレームは 1 つの画面だけを報告し anchor が安定します。
- `SafeRect`: safe area を報告しない画面が、毎回「変わった画面」として扱われなくなりました。空の safe area の代わりに使う全画面 rect を適用済みの値として記録していたため、記録した rect が画面の報告と一致することはなく、`IsScreenChanged()` が true のままとなり、コンポーネントが受け取るすべてのメッセージで safe area を適用し直していました。現在は読み取った値をそのまま記録し、代替の rect は従来どおり anchor の計算と `OnCalculated(Rect)` に渡されます。
- `ScreenChangeNotifier`: play mode の実行中はエディタループが画面を読まなくなりました。play mode は player loop から画面を読んでいるため、両方のループが同じフレームを読み、異なる 2 つの答えで画面を二重に報告することがありました。
...
- `UIAreaGrid.DrawAreaGizmo` と `UIAreaGrid.AreaGizmoColor`: 9 つの領域の外枠を scene view に描く機能を追加しました。既定は有効で、scene view の gizmo 表示設定に従います。`OnDrawGizmos` で grid の local 空間に描くため、回転と scale にそのまま追従します。Screen Space - Overlay canvas は gizmo pass の後に描画されるため、その canvas では UI が覆った部分の外枠が隠れます。
- `UIAreaGrid.HorizontalEditMode` と `UIAreaGrid.VerticalEditMode`: 1 本の分割線が及ぼす範囲を決める軸ごとの `UIAreaEditMode` を追加しました。`Uniform` は grid 全体が 1 組の分割線を共有するため、上の領域を右へ広げると右のライン全体が狭くなります。`Independent` は row（横）と column（縦）ごとに分割線を持つため、同じ編集で右上の領域だけが狭くなります。軸を `Independent` に切り替えると共有の分割線を各 row/column に先に複製するため、配置が飛びません。
- `UIAreaGridEditor` と `UIAreaElementEditor`: 2 つの component の inspector を追加しました。min/max slider で分割線を動かし、別の element が既に持つ領域が無効化される 3x3 の格子から領域を選び、不足している element を生成し、各 element の object と有効状態、fill 状態を grid から直接調整し、影響を受ける子の anchor を `Undo` に記録しながら適用します。
- `SafeCanvas` と `SafeRect`: safe area の component を追加しました。`SafeRect` は自身の RectTransform を `Screen.safeArea` に合わせて anchor を取り、大きさや向きが変わる画面に追従します。`SafeCanvas` は自身の canvas に `SafeRect` を 1 つ用意します。`SafeRect.Calculate()` で safe area を手動で適用でき、`OnCalculated(Rect)` が適用された領域を派生 class に渡します。anchor は親を基準とするため、直下の子にある `SafeRect` だけが canvas のものと認められます。これは `UIAreaElement` が直上の親の `UIAreaGrid` だけに従うのと同じです。
- `SafeCanvas.SafeRect`、`SafeCanvas.CreateSafeRect()`、`SafeCanvas.ResolveSafeRect()`: `SafeRect` は canvas が持つ safe rect を報告し、無い間は null を返すだけで生成はしません。`CreateSafeRect()` は手動で生成しますが、既に持っている場合は 2 つ目を作らずそれを返し、`ResolveSafeRect()` は不足しているものを生成して画面に合わせます。
- `SafeRect`: component が所有する項目を `DrivenRectTransformTracker` で報告するため、RectTransform の inspector では anchor と位置、大きさ、pivot が読み取り専用として表示されます。回転と scale は駆動しません。edit mode でも自身を合わせるので、editor は player と同じ配置を示し、駆動する項目は scene ファイルに書き込まれません。
- `ScreenChangeNotifier`: 画面の大きさや safe area が変わると `OnChanged` を知らせ、確認した値を `ScreenSize` と `SafeArea` で報告する画面通知を追加しました。Unity は `Screen.safeArea` の event を用意していないため、listener ごとに確認する代わりに、この 1 か所からフレームごとに 1 回だけ画面を読みます。play mode では `Application.onBeforeRender` を、edit mode では editor loop を使います。例外を投げる listener は `Debug.LogException` で報告され、残りの listener が画面に追従することを妨げません。
